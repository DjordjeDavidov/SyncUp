...
import { Prisma, profile_visibility, social_mode } from "@/lib/prisma-generated";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import { FormState, fromZodError, profileSchema, settingsSchema } from "@/lib/validation";
...
export async function updateSettingsAction(_: FormState, formData: FormData): Promise<FormState> {
  ...
  const parsed = settingsSchema.safeParse({
    ...
    city: formData.get("city") || "",
    country: formData.get("country") || "",
    social_mode: String(formData.get("social_mode") || currentUser.profile?.social_mode || social_mode.JUST_CHAT),
    profile_visibility: String(
      formData.get("profile_visibility") || currentUser.profile?.profile_visibility || profile_visibility.PUBLIC,
    ),
  });
  ...
  try {
    ...
    return {
      message: error instanceof Error ? error.message : "Could not save your settings right now.",
    };
  }
